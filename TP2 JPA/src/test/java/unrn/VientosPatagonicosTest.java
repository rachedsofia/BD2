...
class VientosPatagonicasTest {
    ...
    void testCalcularMontoTotalSinDescuentosVigentesConDescuentosCaducados() {
        promocionMarca = new PromocionMarca(LocalDate.now().minusDays(10), LocalDate.now().minusDays(5), "Acme", 5);
    ...
    void testCalcularMontoTotalConDescuentoVigenteMarcaAcme() {
        promocionMarca = new PromocionMarca(LocalDate.now().minusDays(1), LocalDate.now().plusDays(1), "Acme", 5);
        descuentos.add(promocionMarca);
        carrito.agregarProducto(producto1);
        carrito.agregarProducto(producto2);
        ProcesoDePago procesoDePago = new ProcesoDePago();
...
de Medio de pago.*/
    ...
    void testCalcularMontoTotalConDescuentoVigenteMedioPago() {
        promocionMedioDePago = new PromocionMedioDePago(LocalDate.now().minusDays(2), LocalDate.now().plusDays(3), "MemeCard", 8);
    ...
    void testCalcularMontoTotalConDosDescuentosVigentes() {
        promocionMarca = new PromocionMarca(LocalDate.now().minusDays(1), LocalDate.now().plusDays(1), "Comarca", 10);
        promocionMedioDePago = new PromocionMedioDePago(LocalDate.now().minusDays(1), LocalDate.now().plusDays(1), "MemeCard", 8);
        descuentos.add(promocionMarca);
        descuentos.add(promocionMedioDePago);
        carrito.agregarProducto(producto1);
        carrito.agregarProducto(producto2);
...
descripción y precio.*/
    ...
    void testCrearDescuentoConFechasSuperpuestas() {
        ...
        PromocionMarca descuento1 = new PromocionMarca(LocalDate.now().minusDays(1), LocalDate.now().plusDays(1), "Acme", 5);
        PromocionMarca descuento2 = new PromocionMarca(LocalDate.now().minusDays(2), LocalDate.now().plusDays(3), "Acme", 10);
